    ...
    <div className="flex flex-col">
      ...
      <section className="bg-wdd-yellow min-h-screen grid grid-cols-2 relative overflow-hidden">
        ...
        <div className="px-16 py-20 flex flex-col justify-center relative z-10">
          <div className="flex items-center gap-3 mb-7">
            ...
            <span className="text-xs font-light tracking-widest text-wdd-black/50 uppercase">{t('hero_tag')}</span>
          </div>
          ...
          <div className="flex gap-10 pt-9 border-t border-wdd-black/12">
            {[['5', t('stats_ateliers')],['500+', t('stats_chantiers')],['11', t('stats_provinces')]].map(([n,l]) => (
              <div key={l}>
      ...
      <section className="bg-wdd-clay py-24 px-16">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-xs font-light tracking-widest text-wdd-ground uppercase">{t('av_eyebrow')}</span>
        </div>
        <h2 className="text-4xl font-bold mb-16">{t('av_title').split(' la ')[0]} <span className="text-wdd-mud">la {t('av_title').split(' la ')[1]}</span></h2>
        <div className="grid gap-0.5" style={{gridTemplateColumns:'1fr 1.4fr 1fr'}}>
          <div className="flex flex-col gap-0.5">
            {[
              { n:'01', bg:'bg-wdd-yellow text-wdd-black', icon:'EUR', title:t('av_1_title'), desc:t('av_1_desc') },
              { n:'02', bg:'bg-wdd-grass text-white', icon:'CO2', title:t('av_2_title'), desc:t('av_2_desc') },
            ].map(card => (
          ...
          <div className="relative overflow-hidden min-h-80 flex flex-col items-center justify-end p-6 bg-wdd-ground">
            ...
            <span className="relative z-10 text-xs font-light text-white/50 tracking-widest uppercase">{t('hero_tag')}</span>
          </div>
          <div className="flex flex-col gap-0.5">
            <div className="bg-white p-9 relative overflow-hidden group hover:bg-wdd-black transition-colors duration-200 flex-1">
              ...
              <h3 className="text-sm font-semibold mb-2 group-hover:text-white transition-colors">{t('av_3_title')}</h3>
              <p className="text-xs font-light leading-relaxed text-black/55 group-hover:text-white/45 transition-colors">{t('av_3_desc')}</p>
              <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-wdd-yellow scale-x-0 group-hover:scale-x-100 transition-transform origin-left" />
            </div>
            <Link href={getLocalizedPath(loc, 'calculateur')} className="bg-wdd-yellow p-9 flex flex-col justify-between group hover:bg-wdd-black transition-colors duration-200 cursor-pointer flex-1">
              <div className="text-3xl font-bold text-wdd-black group-hover:text-white transition-colors leading-tight">{t('av_calc')}</div>
              <span className="text-xs font-light text-wdd-black/55 group-hover:text-white/45 transition-colors mt-3">{t('av_calc_sub')} +</span>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-24 px-16">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-5 h-0.5 bg-wdd-yellow" />
          <span className="text-xs font-light tracking-widest text-wdd-ground uppercase">{t('svc_eyebrow')}</span>
        </div>
        <h2 className="text-4xl font-bold mb-16">{t('svc_title').split(' une ')[0]}, <span className="text-wdd-mud">une {t('svc_title').split(' une ')[1]}</span></h2>
        <div className="grid grid-cols-2 gap-0.5">
          {[
            { badge:t('svc_1_badge'), badgeCls:'bg-wdd-yellow text-wdd-black', title:t('svc_1_title'), desc:t('svc_1_desc'), href:getLocalizedPath(loc,'geo_fermee'), dot:'bg-wdd-yellow', imgBg:'bg-wdd-ground/50' },
            { badge:t('svc_2_badge'), badgeCls:'bg-wdd-mud text-white', title:t('svc_2_title'), desc:t('svc_2_desc'), href:getLocalizedPath(loc,'geo_ouverte'), dot:'bg-wdd-mud', imgBg:'bg-wdd-mud/40' },
          ].map(svc => (
            <div key={svc.badge} className="overflow-hidden group cursor-pointer">
              ...
              <div className="bg-wdd-clay p-10 group-hover:bg-wdd-black transition-colors duration-200">
                ...
                <p className="text-xs font-light text-black/60 leading-relaxed mb-6 group-hover:text-white/40 transition-colors">{svc.desc}</p>
      ...
      <section className="grid grid-cols-2">
        <div className="grid grid-cols-2">
          <div className="bg-wdd-yellow p-14 flex flex-col justify-between min-h-96">
            ...
            <Link href={getLocalizedPath(loc,'devis')} className="inline-block bg-wdd-black text-wdd-yellow px-5 py-3 text-xs font-bold border-2 border-wdd-black hover:bg-transparent hover:text-wdd-black transition-colors self-start mt-6">{t('hero_cta')} +</Link>
          </div>
      ...
      <section className="bg-white py-16 px-16">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-5 h-0.5 bg-wdd-yellow" />
          <span className="text-xs font-light tracking-widest text-wdd-ground uppercase">{t('clients_eyebrow')}</span>
        </div>
        <h2 className="text-3xl font-bold mb-10">Nos <span className="text-wdd-mud">{t('clients_title').split('Nos ')[1] || t('clients_title')}</span></h2>
      ...
      <section className="bg-wdd-clay py-24 px-16">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-5 h-0.5 bg-wdd-yellow" />
          <span className="text-xs font-light tracking-widest text-wdd-ground uppercase">{t('blog_eyebrow')}</span>
        </div>
          ...
          {[
            ...
            <div key={post.title} className="overflow-hidden group cursor-pointer">
              ...
              <div className="bg-white p-6 group-hover:bg-wdd-yellow transition-colors">
                ...
                <span className="text-xs font-bold mt-3 block">{t('blog_read')} +</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="bg-wdd-yellow grid grid-cols-2 min-h-72">
        ...
        <div className="px-14 py-16 flex flex-col justify-center">
          <div className="text-xs font-light tracking-widest text-wdd-black/40 uppercase mb-4">{t('cta_pre')}</div>
          <h2 className="text-4xl font-bold text-wdd-black mb-8 leading-tight">{t('cta_title')}</h2>
          <div className="flex flex-col items-start gap-4">
            <Link href={getLocalizedPath(loc,'devis')} className="bg-wdd-black text-wdd-yellow px-8 py-4 text-sm font-bold border-2 border-wdd-black hover:bg-transparent hover:text-wdd-black transition-colors">{t('cta_btn')}</Link>
