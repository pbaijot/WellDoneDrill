...
export default function HomeTunnel() {
  ...
  const devisPath = getLocalizedPath(locale as any, 'devis')
  const soumissionPath = getLocalizedPath(locale as any, 'pro_soumission')

  return (
    <div className="w-full max-w-md">
      <div className="mb-6">
        <h2 className="text-lg font-bold text-white mb-1">Mon projet geothermique</h2>
        <p className="text-xs font-light text-white/40 leading-relaxed">3 questions pour evaluer votre projet et obtenir une estimation.</p>
      </div>

      {step === 0 && (
        <div className="grid grid-cols-2 gap-1">
          {([['part', 'Particulier', 'Maison ou appartement'], ['pro', 'Professionnel', 'Entreprise ou institution']] as const).map(([p, label, desc]) => (
            <button key={p} onClick={() => choose(p)} className="bg-white/5 border-2 border-transparent hover:border-wdd-yellow hover:bg-black/20 p-6 text-left transition-all group">
              <div className="text-2xl mb-3">{p === 'part' ? 'H' : 'E'}</div>
              <div className="text-sm font-semibold text-white mb-1">{label}</div>
              <div className="text-xs font-light text-white/35">{desc}</div>
          ...
          <div className="border-t-2 border-wdd-yellow bg-white/5 p-6">
            {profile === 'part' ? (
              <>
                ...
                <div className="text-xs font-light text-white/50 leading-relaxed mb-5">Systeme de geothermie fermee avec 2 a 4 sondes de 100m. Devis precis gratuit sous 48h.</div>
              </>
            ) : (
              <>
            ...
            <Link href={profile === 'part' ? devisPath : soumissionPath} className="block w-full py-3 bg-wdd-yellow text-wdd-black text-sm font-bold text-center hover:bg-wdd-yellow/90 transition-colors">
              {profile === 'part' ? 'Obtenir mon devis precis +' : 'Soumettre mon projet +'}
            </Link>
            <a href="tel:+32494142449" className="block w-full py-2.5 mt-0.5 border border-white/10 text-white/40 text-xs text-center hover:border-white/30 hover:text-white transition-colors">
